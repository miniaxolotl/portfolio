...
import { ArrowDownUp, Download, FolderOpen, Home, Moon, Sun, TreePine, X } from "lucide-react";
...
export const MobileSidebar = observer(({ onNavigate }: MobileSidebarProps) => {
  ...
  return (
    <div className="flex flex-col h-full" role="dialog" aria-label="Mobile navigation">
      <div className="flex-1 overflow-y-auto overscroll-y-contain" style={{ WebkitOverflowScrolling: "touch" }}>
        ...
        <div className="relative px-6 pt-6 pb-4">
          ...
          <div className="flex flex-col items-center pt-2">
            <Link href="/" onClick={onNavigate} className="flex flex-col items-center gap-1">
        ...
        <div className="px-4 py-2">
          <p className="text-xs font-medium text-muted-foreground/60 uppercase tracking-wider mb-2 px-2">Pages</p>
        ...
        {/* Actions */}
        <div className="px-4 py-2">
          <p className="text-xs font-medium text-muted-foreground/60 uppercase tracking-wider mb-2 px-2">Actions</p>
          <div className="flex flex-col gap-0.5">
            ...
            >
              <span className="flex items-center justify-center w-8 h-8 rounded-md bg-background text-muted-foreground group-hover:text-foreground transition-colors">
                {mounted ? themeStore.isDark ? <Sun size={16} /> : <Moon size={16} /> : <span className="h-4 w-4" />}
              </span>
              <span>{mounted ? (themeStore.isDark ? "Switch to light" : "Switch to dark") : "Theme"}</span>
            </button>
          </div>
        </div>
